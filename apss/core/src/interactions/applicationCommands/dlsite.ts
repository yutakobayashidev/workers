...
const handler = async ({
	...
	try {
		const details = (await fetch(
			`https://www.dlsite.com/maniax/api/=/product.json?workno=${code}`,
		...
		if (details.length > 0) {
			...
			const embed: APIEmbed = {
				...
				fields: [
					{
						name: "販売日",
						value: new Date(details[0].regist_date).toLocaleDateString(),
					},
					{
						name: "価格",
						value: `${details[0].price.toLocaleString()}円 / ${details[0].point}pt`,
					},
			...
			embed.fields.push({
				name: "ファイル容量",
				value: `総計 ${details[0].contents[0].file_size_unit}`,
			});
