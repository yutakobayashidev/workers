...
async function fetchDoodleData(
	...
	try {
		const response = await fetch(
			`https://www.google.com/doodles/json/${year}/${month}?hl=${LANGUAGE}`,
		);
		const doodles: any = await response.json();

		for (const doodle of doodles) {
			await postDoodleToDiscord({
				authKey,
				doodle,
				DISCORD_TOKEN,
...
async function postDoodleToDiscord({
	...
	const description = await getDoodleDescription(
		authKey,
		`https://doodles.google/doodle/${doodle.name}`,
	);

	const client = new DiscordClient(DISCORD_TOKEN, DISCORD_APPLICATION_ID);

	const colorIndex = await YUTA_STUDIO.get("google_color");
	const color = getColor(Number.parseInt(colorIndex ?? "0"));
	await YUTA_STUDIO.put(
		"google_color",
		String((Number.parseInt(colorIndex ?? "0") + 1) % 4),
	);
